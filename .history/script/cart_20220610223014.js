...
popupCart.addEventListener("click", (e) => {
  if (e.target.classList.contains("icon-remove")) {
    ...
    console.log(cartItem.parentNode.removeChild(cartItem));
    const id = e.target.parentNode.dataset.productId;
    console.log(
      "🚀 ~ file: cart.js ~ line 76 ~ popupCart.addEventListener ~ id",
      id
    );
    const cart = [...productCart].filter((item) => item.id !== Number(id));
    console.log(
